...
class CustomLoss(Module):
    def __init__(self, target_letter='e'):
    ...
    def forward(self, logits, labels):
        ...
        next_token_id = torch.multinomial(torch.nn.functional.softmax(next_token_logits / 0.8, dim=-1), 1)
        next_token = [tokenizer.decode(token_id, skip_special_tokens=True) for token_id in next_token_id ]
        # Calculate the custom loss based on occurrences of the target letter
        print(next_token)
        occurrences = torch.tensor((c == self.target_letter for c in next_token),dtype=bool,requires_grad=True).sum()
        print(torch.tensor((c == self.target_letter for c in next_token),dtype=bool,requires_grad=True))
        #occurrences = torch.tensor(labels == ord(self.target_letter)).sum()
        
        # You can customize the loss calculation based on your specific requirements
        # In this example, I'm simply using the number of occurrences as the loss
        loss = occurrences
...
def fine_tune(training_data, sentiment_target, num_epochs, lr, batch_size, num_tokens_to_generate=15):
    ...
    target_letter = 'e'
    custom_loss_fn = CustomLoss(target_letter=target_letter)
